...
from summary_features.calculate_summary_features import (
    calculate_summary_statistics_alternative as alternative_sumstats,
)
from summary_features.calculate_summary_features import (
    calculate_summary_stats_temporal as temporal_sumstats,
)
from summary_features.calculate_summary_features import (
    calculate_summary_stats_number as number_sumstats,
)
...
plt.fill_between(
    x=torch.arange(len(mean_prior)), y1=lower, y2=upper, color="blue", alpha=0.1
)
plt.xlim(0, 7000)
...
for i in range(len(sum_stats_names)):
    ...
    globals()["ax%s" % i].hist(
        ...
        alpha=0.5,
        histtype="barstacked",
        label="from proposal/prior",
    )

    globals()["ax%s" % i].set_title(
        'Histogram of summary stat "{}" '.format(sum_stats_names[i]), pad=20
    )
    # ax0.set(ylim=(-500, 7000))

    globals()["ax%s" % i].axvline(
        obs_real_stat[0][i].detach().numpy(), color="red", label="ground truth"
    )
