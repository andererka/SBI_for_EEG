...
def main(argv):
    ...
    try:
        ...
        theta, x_without = inference.run_sim_theta_x(
            prior1, sim_wrapper, num_simulations=num_sim, num_workers=num_workers
        )
    ...
    obs_real = inference.run_only_sim(
        torch.tensor([list(true_params[0][0:1])]), sim_wrapper, num_workers=num_workers
    )
    # obs_real = inference.run_only_sim(
    ...
    s_x = inference.run_only_sim(
        samples, simulation_wrapper=sim_wrapper, num_workers=num_workers
    )
    ...
    file_writer.save_all(
        start_time=start_time, finish_time=finish_time, source="sequential_inference"
    )
