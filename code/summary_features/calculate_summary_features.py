...
def calculate_summary_stats3(x):
    """
    Input: observations or simulations
    Returns summary statistics 
    specific for event related potentials
    ...
    for batch in x:
        ...
        arg_p50 = torch.tensor(torch.argmax(batch[0:arg70ms]), dtype=torch.float32)
        ...
        arg_N100 = torch.tensor(torch.argmin(batch[:arg200ms]), dtype=torch.float32)

        arg_P200 = torch.tensor(torch.argmax(batch[arg70ms:]), dtype=torch.float32)
...
def calculate_summary_stats12(x):
    """
    Input: observations or simulations
    Returns summary statistics 
    specific for event related potentials
    ...
    for batch in x:
        ...
        p50_moment2 = torch.tensor(
            moment(batch[0:arg70ms], moment=2), dtype=torch.float32
        )  # variance
        # p50_moment3 = torch.tensor(moment(x[0:arg70ms], moment=3) )  #skewness
        # p50_moment4 = torch.tensor(moment(x[0:arg70ms], moment=4) )  #kurtosis
        ...
        N100_moment2 = torch.tensor(
            moment(batch[0:arg200ms], moment=2), dtype=torch.float32
        )  # variance
        # N100_moment3 = torch.tensor(moment(x[0:arg200ms], moment=3))    #skewness
        # N100_moment4 = torch.tensor(moment(x[0:arg200ms], moment=4))    #kurtosis
        ...
        P200_moment2 = torch.tensor(
            moment(batch[arg70ms:], moment=2), dtype=torch.float32
        )  # variance
        # P200_moment3 = torch.tensor(moment(x[arg70ms:], moment=3))   #skewness
        # P200_moment4 = torch.tensor(moment(x[arg70ms:], moment=4))    #kurtosis
        ...
        sum_stats_vec = torch.stack(
            [
                ...
                arg_P200,
                p50_moment1,
                p50_moment2,  # p50_moment3, #p50_moment4,
                N100_moment1,
                N100_moment2,  # N100_moment3, #N100_moment4,
                P200_moment1,
                P200_moment2,  # P200_moment3, #P200_moment4
            ]
        )
...
def calculate_summary_stats_P200(x):
    ...
    for batch in x:
        ...
        sum_stats_vec = torch.stack(
            [P200, arg_P200, P200_moment1,]  # P200_moment2, P200_moment3, #P200_moment4
        )
