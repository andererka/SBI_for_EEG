...
def calculate_summary_stats_number(x, number_stats):
    ...
    for batch in x:
        ...
        if number_stats == 6:
            ...
            p50_moment1 = torch.mean(
                batch[arg_p50 - 10 * time_window : arg_p50 + 10 * time_window]
            )  # mean
            N100_moment1 = torch.mean(
                batch[arg_N100 - 10 * time_window : arg200ms + 10 * time_window]
            )  # mean
            P200_moment1 = torch.mean(
                batch[arg_P200 - 10 * time_window : arg_P200 + 10 * time_window]
            )  # mean

            p50_moment2 = torch.var(
                batch[arg_p50 - 10 * time_window : arg_p50 + 10 * time_window]
            )  # variance
            N100_moment2 = torch.var(
                batch[arg_N100 - 10 * time_window : arg200ms + 10 * time_window]
            )  # variance
            P200_moment2 = torch.var(
                batch[arg_P200 - 10 * time_window : arg_P200 + 10 * time_window]
            )  # variance
            ...
            area_p50 = trapz(
                batch[:zero_cross_p50], dx=1
            )  # Integrate along the given axis using the composite trapezoidal rule. dx is the spacing between sample points
            ...
            # compute area under the curve:
            area_N100 = trapz(
                batch[zero_cross_p50:zero_cross_N100], dx=1
            )  # Integrate along the given axis using the composite trapezoidal rule. dx is the spacing between sample points
            area_N100 = torch.tensor(area_N100)

            # compute area under the curve:
            area_P200 = trapz(
                batch[zero_cross_N100:], dx=1
            )  # Integrate along the given axis using the composite trapezoidal rule. dx is the spacing between sample points
            ...
            p50_moment2 = torch.tensor(
                moment(
                    batch[arg_p50 - 10 * time_window : arg_p50 + 10 * time_window],
                    moment=2,
                )
            )  # variance
            N100_moment2 = torch.tensor(
                moment(
                    batch[arg_N100 - 10 * time_window : arg200ms + 10 * time_window],
                    moment=2,
                )
            )  # variance
            P200_moment2 = torch.tensor(
                moment(
                    batch[arg_P200 - 10 * time_window : arg_P200 + 10 * time_window],
                    moment=2,
                )
            ...
            p50_moment3 = torch.tensor(
                moment(
                    batch[arg_p50 - 10 * time_window : arg_p50 + 10 * time_window],
                    moment=3,
                )
            )  # skewness
            N100_moment3 = torch.tensor(
                moment(
                    batch[arg_N100 - 10 * time_window : arg200ms + 10 * time_window],
                    moment=3,
                )
            )  # skewness
            P200_moment3 = torch.tensor(
                moment(
                    batch[arg_P200 - 10 * time_window : arg_P200 + 10 * time_window],
                    moment=3,
                )
            ...
            p50_moment4 = torch.tensor(
                moment(
                    batch[arg_p50 - 10 * time_window : arg_p50 + 10 * time_window],
                    moment=4,
                )
            )  # kurtosis
            N100_moment4 = torch.tensor(
                moment(
                    batch[arg_N100 - 10 * time_window : arg200ms + 10 * time_window],
                    moment=4,
                )
            )  # kurtosis
            P200_moment4 = torch.tensor(
                moment(
                    batch[arg_P200 - 10 * time_window : arg_P200 + 10 * time_window],
                    moment=4,
                )
...
def calculate_summary_stats_temporal(x):
    ...
    for batch in x:
        ...
        N100 = torch.min(batch)
        N100_moment1 = torch.mean(
            batch[arg_N100 - 10 * time_window : arg_N100 + 10 * time_window]
        )  # mean
        N100_moment2 = torch.var(
            batch[arg_N100 - 10 * time_window : arg_N100 + 10 * time_window]
        )  # variance
