...
def main(argv):
    ...
    prior1 = utils.torchutils.BoxUniform(low=prior_min[0:7], high=prior_max[0:7])
    ...
    shutil.copyfile(
        str(os.getcwd() + "/sequential_inference_17params.py"),
        str(file_writer.folder + "/sequential_inference_17params.py"),
    )
    ...
    json_dict = {"inference time for last step:": str(inf_time1 - start_inference_time)}
    ...
    json_dict = {"inference time for last step:": str(inf_time2 - finish_time)}
    with open("inference_time.json", "a") as f:
        json.dump(json_dict, f)
        f.close()
    ...
    json_dict = {"inference time for last step:": str(end_time - finish_time)}
    with open("inference_time.json", "a") as f:
        json.dump(json_dict, f)
        f.close()
